...
open class LoopingLayoutManager : LayoutManager, RecyclerView.SmoothScroller.ScrollVectorProvider {
	 ...
	 */
	constructor(orientation: Int = VERTICAL, reverseLayout: Boolean = false) {
	...
	override fun onLayoutChildren(recycler: RecyclerView.Recycler, state: RecyclerView.State) {
		...
		while (sizeFilled < size) {
			...
			layoutRect =
				prevItem?.getPositionOfItemFollowingSelf(item, layoutRect) ?: item.getPositionOfSelfAsFirst(
					layoutRect,
					layoutRequest.scrollOffset
				)
			layoutDecorated(
	 ...
	 * return less than the delta if they hit a bound, but the boilerplate.widget.recyclerview.layoutManager.LoopingLayoutManager has no
	 * bounds.
	 */
	 ...
	 * scrollbars (which the boilerplate.widget.recyclerview.layoutManager.LoopingLayoutManager does not support), but it is also needed to
	 ...
	 * the boilerplate.widget.recyclerview.layoutManager.LoopingLayoutManager does not support), but it is also needed to support TalkBack
	...
	/**
	 * A smooth scroller that supports the boilerplate.widget.recyclerview.layoutManager.LoopingLayoutManager's two (at the time of writing) quirks:
	...
	private inner class LoopingSmoothScroller(
		...
		/**
		 * Tells the boilerplate.widget.recyclerview.layoutManager.LoopingLayoutManager to start laying out extra (i.e. not visible) views. This
		...
		/**
		 * Tells the boilerplate.widget.recyclerview.layoutManager.LoopingLayoutManager to stop laying out extra views, b/c there's no need
		...
		override fun computeScrollVectorForPosition(targetPosition: Int): PointF? {
			...
			Log.w(
				TAG,
				"A LoopingSmoothScroller should only be attached to a boilerplate.widget.recyclerview.layoutManager.LoopingLayoutManager."
			)
			return null
		}
	}
	...
	companion object {
		...
		private const val TAG = "boilerplate.widget.recyclerview.layoutManager.LoopingLayoutManager"
