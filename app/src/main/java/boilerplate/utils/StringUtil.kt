...
object StringUtil {
	...
	fun handleMessageContent(
		...
		if (message != null) {
			val isMe: Boolean = AccountManager.getCurrentUserId().equals(message.personSendId)
			...
			val content: String =
				getHtml(message.mainContent[0]).toString().trim { it <= ' ' }
			val senderName =
				if (isMe) "Bạn" else if (isGroup) message.personSend.name else ""
			...
			if (message.isWithdraw) {
				...
				val hasQuote: Boolean = message.forwardMessage.isNotEmpty()
				val hasSurveyFile: Boolean = message.surveyFiles.isNotEmpty()
				val hasFile: Boolean = message.attachedFiles.isNotEmpty()
				...
				if (hasSurveyFile) {
					val isBreakFastFile: Boolean = message.surveyFiles[0].isSurveyBreakfast
...
//			val aHexFormatted = String.format("%2s", aHex).replace(" ", "0")
	...
	fun formatQuoteMessage(message: Message, addCurrent: Boolean): ArrayList<Quote> {
		return arrayListOf<Quote>().apply {
			val quote = Quote(message)
			if (message.content.contains(KEY_FORWARD_JSON)) {
				message.mainContent.let {
					quote.content = it[0]
					try {
						val listQuote: ArrayList<Quote> =
							Gson().fromJson(it[1], object : TypeToken<ArrayList<Quote>>() {}.type)
						addAll(listQuote)
