	...
	override fun initialize() {
		val simpleEvent = object : ConversationAdapter.SimpleEvent() {
				...
				showDialog(DialogBaseBinding.inflate(layoutInflater)) { b, dialog ->
					with(b) {
						...
						btnConfirm.click {
							...
							SignalRManager.disableConversation(conversation.id)
							dialog.dismiss()
						}
					}
				}
			...
			override fun onNotify(item: Conversation, isNotify: Boolean) {
				if (isNotify) {
					SignalRManager.turnNotifyConversation(item.id, false)
				} else {
					showDialog(DialogBaseBinding.inflate(layoutInflater)) { view, dialog ->
						with(view) {
							...
							btnConfirm.click {
								_isDeleteConversation = true
								SignalRManager.turnNotifyConversation(item.id, true)
								dialog.dismiss()
							}
						}
					}
			...
			.setListener(object : SignalRImpl() {
				override fun readyToChat(list: ArrayList<Conversation>) {
					...
					for (conversation in list) {
						if (conversation.pinDate.isEmpty()) {
				...
				override fun sendMessage(sendMessage: SendMessageResult) {
					if (sendMessage.status == 1 || sendMessage.status == 0) {
						if (newMessage(sendMessage.entity, true)) {
							viewModel.apiGetConversationDetail(sendMessage.entity)
						}
					}
				}
				...
				override fun importConversation(important: ImportantConversation) {
					...
					if (!_adapterPin.updatePinImportant(conversation.id, isImportant)) {
						_adapter.updateImportant(conversation.id, isImportant)
					}
				}
