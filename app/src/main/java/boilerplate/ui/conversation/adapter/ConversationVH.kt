...
class ConversationVH(
	...
	fun setData(con: Conversation, isShowPin: Boolean) {
		...
		if (con.isMyCloud) {
			...
			var countName = 0
			val iterator: ListIterator<Member> = con.members.listIterator()
		...
		with(_binding) {
			if (unRead <= 0) {
				...
				tvUnread.apply {
					...
					if (unRead > 9) {
						setText(R.string.unread_message_count)
					} else {
	...
	private fun checkImportantMessage(user: Member) {
