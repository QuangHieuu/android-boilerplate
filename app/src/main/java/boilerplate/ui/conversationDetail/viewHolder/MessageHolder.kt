...
abstract class MessageHolder(
	...
	private fun formatMessage() {
		with(_message) {
			val strings: Array<String> = mainContent
	...
	private fun addViewFile(file: AttachedFile, isQuote: Boolean, status: Int): View {
		val name: String = file.getDisplayName(itemView.context).replace("_system_deleted", "")
		...
		if (ExtensionType.isFileAudio(file.fileName) && !isQuote) {
			val binding = ItemFileAudioBinding.inflate(
				...
				imgDownload.click { _listener.openFile(file) }
				imgPlay.click { _listener.playRecord(file) }
			}
			return binding.root
		} else {
			...
			return binding.root.apply {
				click { _listener.openFile(file) }
				setPadding(0, _padding / 2, 0, _padding / 2)
			}
		}
	}

	private fun addViewSurvey(file: AttachedFile): View {
		val layoutInflater = LayoutInflater.from(itemView.context)
		val binding =
	...
	private fun addViewImage(file: AttachedFile, status: Int): View {
		val imageView = RoundedImageView(itemView.context).apply {
			...
			val string = if (status == 2) file.filePath else file.filePreview
