...
class ConversationInformFragment :
	...
	private fun handleAvatar(con: Conversation) {
		with(binding) {
			if (con.isMyCloud) {
				...
				val thumb = con.thumb
				val size = con.totalMember
				val userSize: Int = con.members.size
				val isOneUser = size == 1

				frameUserOnline.show(isGroup)

				rlAvatarGroup.removeAllViews()
				val builder = StringBuilder(if (isGroup) con.name else "")
				...
				for (user in con.members) {
					val index = con.members.indexOf(user)
					if (isGroup) {
	...
	private fun addAvatar(index: Int, size: Int, user: Member?) {
	...
	private fun handleMenu(conversation: Conversation) {
		val size: Int = conversation.totalMember
	...
	private fun handleNotifyOnOff(conversationId: String) {
		checkCurrentConversation(conversationId) {
			viewModel.conversation.value?.apply {
				val listIterator = members.listIterator()
	...
	private fun changeInform() {
		showDialog(DialogConversationInformBinding.inflate(layoutInflater)) { vb, dialog ->
			...
			with(vb) {
				...
				if (!viewModel.conversation.value?.avatar.isNullOrEmpty()) {
				...
				edtName.apply {
					setText(viewModel.conversation.value?.name)
