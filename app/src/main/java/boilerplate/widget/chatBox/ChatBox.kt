...
import boilerplate.utils.StringUtil.MENTION_SIGN
...
class ChatBox @JvmOverloads constructor(
	...
	private val _gson: Gson by KoinJavaComponent.inject<Gson>(Gson::class.java)
	...
	private val _mentions: ArrayList<Member> = arrayListOf()
					...
					override fun onPaste() {
						...
						if (clip != null) {
							...
							if (pasteIntent != null) {
								...
								val message: Message = _gson.fromJson(json, Message::class.java)
	...
	fun setupForChat() {
		...
		_mentionAdapter = MentionAdapter(object : MentionAdapter.OnViewListener {
			override fun onChosen(user: Member) {
				_mentions.add(user)
				...
				val mentionPos = text.toString().lastIndexOf(MENTION_SIGN)
		...
		_boxAdapter = BoxAdapter {
			_boxAdapter.clearQuote(it as Quote)
			checkRecyclerView()
		}
		with(_binding) {
			rcvMention.apply {
	...
	fun addMentions(list: ArrayList<Member>) {
		val mentions = ArrayList<Member>()
		for (s in list) {
			if (s.user.id != AccountManager.getCurrentUserId()) {
				mentions.add(s)
			}
		}
			...
			try {
				if (json.isNotEmpty()) {
					val listQuote: ArrayList<Quote> =
						_gson.fromJson(json, object : TypeToken<ArrayList<Quote?>?>() {}.type)
			...
			if (_hasMentionFunc && isEdit) {
				...
				for (user in _mentionAdapter.originMentions) {
					if (list.contains(user.user.id)) {
						_mentions.add(user)
						val name = MENTION_SIGN + user.user.name
	...
	fun clearMessageQuote() {
		_boxAdapter.clearAll()
	}

	fun handleForwardMessage(forwardMessages: ArrayList<Quote>) {
		if (forwardMessages.size > 0) {
			finishSendingMessage()

			_boxAdapter.addQuote(forwardMessages)
			_binding.btnSendLike.gone()
			_binding.btnSend.show()
			checkRecyclerView()
			checkSendButton(getEditText())
		}
	}

	fun mentionAnswer(userId: String) {
		if (!_hasMentionFunc) {
			return
		}
		for (user in _mentionAdapter.originMentions) {
			val id: String = user.user.id
			if (id == userId && id != AccountManager.getCurrentUserId()) {
				_mentions.clear()
				_mentions.add(user)

				val name: String = (MENTION_SIGN + user.user.name) + " "
				_ranges.add(Range(0, name.length))
				setEditContent(0, name)
				break
			}
		}
	}

	fun editMessage(message: Message, isEdit: Boolean) {
		finishSendingMessage()
		_isEdit = isEdit
		_tempMessage = message
		with(_binding) {
			btnSend.isEnabled = false
			btnCancelEdit.show(_isEdit)
		}
		for (file in message.attachedFiles) {
			_boxAdapter.addFile(file.apply {
				isUpload = false
				isPreventRemove = false
			})
		}
		for (file in message.surveyFiles) {
			_boxAdapter.addFile(file.apply {
		...
		message.forwardMessage.let {
			if (it.isNotEmpty()) {
				try {
					val listQuote = _gson.fromJson<ArrayList<Quote>>(
						it,
		...
		val copyText = StringUtil.getHtml(message.mainContent[0]).toString()
		if (_hasMentionFunc && _isEdit) {
			val list = StringUtil.findUserMentionId(message.mainContent[0])
			for (user in _mentionAdapter.originMentions) {
				if (list.contains(user.user.id)) {
					_mentions.add(user)
					val name = MENTION_SIGN + user.user.name
	...
	fun setEditWithKeyboard(copyText: String) {
		setEditContent(0, copyText)
		if (copyText.isNotEmpty() || _boxAdapter.hasFile() || _boxAdapter.hasQuote()) {
			getEditMessage().showKeyboard()
		}
	}

	private fun setEditContent(posInsert: Int, editContent: String) {
		with(_binding) {
			val currentCursor: Int = edtMessage.selectionStart
	...
	private fun handleInputMention(s: CharSequence) {
		if (_hasMentionFunc) {
			...
			if (checkMention.contains(MENTION_SIGN)) {
				val textSearch: String =
					checkMention.substring(checkMention.lastIndexOf(MENTION_SIGN) + 1)
	...
	private fun filterMention(textSearch: String) {
		val finds = ArrayList<Member>()
		for (userTag in _mentionAdapter.originMentions) {
			val alreadyMention = alreadyMention(userTag.user.id)
			val isContain = userTag.user.name.lowercase().contains(textSearch.lowercase())
			if (!alreadyMention && (isContain || textSearch.isEmpty())) {
				finds.add(userTag)
			}
		}
	...
	private fun mentionStyle() {
		...
		synchronized(_mentions) {
			...
			val iterator: MutableListIterator<Member> = _mentions.listIterator()
	...
	private fun getMessageContent(): String {
		...
		if (messageForward.isNotEmpty()) {
			val jsonForward = _gson.toJson(messageForward)
	...
	private fun getEditText(): String {
		...
		for (mention in _mentions) {
			...
			val mentionString = MENTION_SIGN + user.name
