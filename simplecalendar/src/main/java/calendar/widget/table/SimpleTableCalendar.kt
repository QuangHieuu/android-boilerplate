...
class SimpleTableCalendar(
	...
	private val _textTodayPaint = Paint().apply {
		isAntiAlias = true
		style = Paint.Style.FILL
	}
	private val _textDisable = Paint().apply {
		flags = Paint.STRIKE_THRU_TEXT_FLAG
		style = Paint.Style.FILL
	}
	private val _textSelectedPaint = Paint().apply {
		isAntiAlias = true
		color = Color.WHITE
		style = Paint.Style.FILL
	}
	private val _textHeaderPaint = Paint().apply {
		isAntiAlias = true
		style = Paint.Style.FILL
	}
	private val _textDefaultPaint = Paint().apply {
		isAntiAlias = true
		style = Paint.Style.FILL
	}
	...
	private var _choseOnlyFuture: Boolean = false
	...
	private val _todaySplit: Array<Int>
		get() = CalUtils.getSplitCurrentDay(monthIndex = true)
	...
	private fun drawTable(canvas: Canvas) {
		calculatorTable(canvas, PREVIOUS)
		calculatorTable(canvas, CURRENT)
		calculatorTable(canvas, NEXT)
		if (_isShowStroke) {
			_linePath.reset()
			drawStroke(PREVIOUS)
			drawStroke(CURRENT)
			...
			canvas.drawPath(_linePath, _strokePaint)
			canvas.clipPath(_linePath)
		}
	}

	private fun calculatorTable(canvas: Canvas, offset: Int) {
	...
	private fun drawMonth(canvas: Canvas, offset: Int, year: Int, month: Int) {
		...
		while (dayColumn < CALENDAR_COLUMN) {
			...
			if (dayRow == 0) {
				...
				drawHeader(canvas, xPosition, yPosition, text)
			} else {
				...
				if (position < list.size) {
					...
					when {
						_pickFromTo && dayOfMonth.isInRange -> {
							drawSelectedInRange(canvas, xPosition, yPosition, dayOfMonth)
						}

						dayOfMonth.isSelected -> {
							drawSelectedCircle(canvas, xPosition, yPosition, dayOfMonth)
						}

						else -> drawText(canvas, xPosition, yPosition, dayOfMonth)
					}
				}
			}
			dayRow += 1
	...
	private fun drawStickyHeader(canvas: Canvas) {
		_stickyPath.reset()
		canvas.drawRect(_stickyRect, _stickyBgPaint)
		for (column in 0..CALENDAR_COLUMN) {
			...
			if (column < CALENDAR_COLUMN) {
				...
				drawHeader(canvas, xPosition, 0f, text)
			}
		...
		if (_isShowStroke) {
			canvas.drawPath(_stickyPath, _strokePaint)
		}
	}

	private fun drawHeader(canvas: Canvas, xPosition: Float, yPosition: Float, text: String) {
		val size = computeTextCoordination(text, xPosition, yPosition)
		canvas.drawText(text, size.first, size.second, _textHeaderPaint)
	}
	...
	private fun drawText(
		...
		canvas.drawText(text, size.first, size.second, paint)
	}

	private fun drawSelectedInRange(
		canvas: Canvas,
		xPosition: Float,
		yPosition: Float,
		dayOfMonth: DayOfMonth,
		...
		_rangePath.addRect(
			...
			yPosition.plus(_cellHeight).plus(_strokePaint.strokeWidth),
			Path.Direction.CCW
		)
		canvas.drawPath(_rangePath, _selectedRangePaint)
		drawText(canvas, xPosition, yPosition, dayOfMonth)
	}

	private fun drawSelectedCircle(
		canvas: Canvas,
		xPosition: Float,
		yPosition: Float,
		dayOfMonth: DayOfMonth,
		...
		if (compareSameTwoDate(dayOfMonth.toDate, _selected) && _selectedTo != null) {
			_rangePath.addCircle(
				xPosition.plus(_cellWidth.div(2)),
				yPosition.plus(_cellHeight.div(2)).plus(_density),
				_cellHeight.div(2f),
				Path.Direction.CCW
			)
			_rangePath.addRect(
		...
		canvas.drawCircle(
			xPosition.plus(_cellWidth.div(2)),
			yPosition.plus(_cellHeight.div(2)).plus(_density),
			_cellHeight.div(2f).minus(_density.times(3)),
			_selectedPaint
		)
		drawText(canvas, xPosition, yPosition, dayOfMonth)
	}
	...
	private fun initDataMonth(month: Int, year: Int): List<DayOfMonth> {
		...
		fun checkIsSelected(date: DayOfMonth): Boolean {
			return _selected?.let {
				val split = CalUtils.getSplitDay(it.toString(), monthIndex = true)
				split[0] == date.day && split[1] == date.month && split[2] == date.year
			} == true || _selectedTo?.let {
				val split = CalUtils.getSplitDay(it.toString(), monthIndex = true)
				split[0] == date.day && split[1] == date.month && split[2] == date.year && _pickFromTo
			} == true
		}
	...
	private fun createHolder(width: Int, height: Int): Bitmap {
		return createBitmap(width, height).let {
			_holderPath.addRoundRect(_holderRect.toRectF(), _radius, _radius, Path.Direction.CCW)

			with(Canvas(it)) {
				drawPath(_holderPath, _holderBgPaint)
				drawPath(_holderPath, _strokePaint)
				clipPath(_holderPath)
			}
			it
		}
